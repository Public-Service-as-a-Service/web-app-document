      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <section className="rounded-xl bg-card p-6 shadow-sm space-y-5">
          <div className="space-y-2">
            <Label htmlFor="description">
              {t('common:document_create_description_label')} <span className="text-destructive">*</span>
            </Label>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div className="space-y-2">
              <Label htmlFor="createdBy">
                {t('common:document_create_created_by_label')} <span className="text-destructive">*</span>
              </Label>
              <Input id="createdBy" className="w-full" value={createdBy} onChange={(e) => setCreatedBy(e.target.value)} />
            </div>
          </div>

          <div className="flex items-center gap-3">
            <Switch id="confidential" checked={confidential} onCheckedChange={setConfidential} />
            <Label htmlFor="confidential" className="cursor-pointer">{t('common:document_create_confidential_label')}</Label>
          </div>
          {confidential && (
            <div className="space-y-2 md:w-1/2">
              <Label htmlFor="legalCitation">{t('common:document_legal_citation')}</Label>
              <Input id="legalCitation" className="w-full" value={legalCitation} onChange={(e) => setLegalCitation(e.target.value)} placeholder="25 kap. 1 § OSL" />
            </div>
          )}
        </section>
        ...
        <section className="rounded-xl bg-card p-6 shadow-sm">
          <div className="mb-3 flex items-center justify-between">
            <h3 className="text-base font-semibold">{t('common:document_create_metadata_label')}</h3>
            ...
            <div className="space-y-2">
              {metadataList.map((m, i) => (
                <div key={i} className="flex items-center gap-2">
                  <Input className="flex-1" placeholder={t('common:document_metadata_key')} value={m.key} onChange={(e) => updateMetadata(i, 'key', e.target.value)} />
                  <Input className="flex-1" placeholder={t('common:document_metadata_value')} value={m.value} onChange={(e) => updateMetadata(i, 'value', e.target.value)} />
                  <Button variant="ghost" size="icon" aria-label={`Ta bort metadata ${m.key || `rad ${i + 1}`}`} onClick={() => removeMetadata(i)} type="button">
        ...
        <section className="rounded-xl bg-card p-6 shadow-sm">
          <h3 className="mb-3 text-base font-semibold">{t('common:document_create_files_label')}</h3>
          <div
            ...
            <div className="mt-3 space-y-1.5">
              {files.map((f, i) => (
                <div key={i} className="flex items-center justify-between rounded-lg bg-muted px-3 py-2">
                  <span className="text-sm">{f.name} <span className="text-muted-foreground">({(f.size / 1024).toFixed(0)} KB)</span></span>
                  <button type="button" onClick={() => removeFile(i)} className="text-muted-foreground hover:text-destructive" aria-label={`Ta bort fil ${f.name}`}>
                    <X size={16} />
                  </button>
                </div>
              ))}
